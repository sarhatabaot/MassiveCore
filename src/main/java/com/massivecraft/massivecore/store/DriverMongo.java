...
public class DriverMongo extends DriverAbstract {
	...
	protected DB getDbInner(String uri) {
		...
		//MongoCredential mongoCredential = MongoCredential.createCredential(muri.getUsername(), muri.getUsername(), muri.getPassword());
		try (MongoClient mongoClient = new MongoClient(muri)){
			...
			/*if (!db.authenticate(muri.getUsername(), muri.getPassword())) { authenticates on creation..
